...
import { TrendingUp, Award, Zap, Store, Activity, Loader2 } from 'lucide-react'
...
export default function AnalyticsPage() {
    ...
    return (
      <div className="flex flex-1 items-center justify-center p-6 text-center">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">{t('analytics_select_store_prompt')}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 md:gap-8 md:p-8 overflow-x-hidden">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">{t('analytics_title')}</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        <Card className="border border-border/60">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              {t('analytics_total_issued')}
            </CardTitle>
            <TrendingUp className="h-4 w-4 text-emerald-500" />
          </CardHeader>
          <CardContent>
            <div className="text-xl sm:text-2xl font-bold">{totalIssued.toLocaleString()}</div>
            <p className="text-[11px] text-muted-foreground mt-0.5">{t('analytics_total_issued_desc')}</p>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border/60">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              {t('analytics_net_outstanding')}
            </CardTitle>
            <Zap className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-xl sm:text-2xl font-bold">{netOutstanding.toLocaleString()}</div>
            <p className="text-[11px] text-muted-foreground mt-0.5">{t('analytics_net_outstanding_desc')}</p>
          </CardContent>
        </Card>
      </div>
      ...
      {loading && !analytics ? (
            ...
            <div className="h-[260px] sm:h-[340px] md:h-[400px] w-full mt-2">
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Bar dataKey="issued" name={t('analytics_issued_legend')} fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="redeemed" name={t('analytics_redeemed_legend')} fill="hsl(var(--destructive))" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="border border-border/60">
          <CardHeader>
            <CardTitle className="text-base">{t('analytics_recent_activity_title')}</CardTitle>
            <CardDescription className="text-xs">
              {t('analytics_recent_activity_desc')}
            </CardDescription>
          </CardHeader>
          <CardContent>
