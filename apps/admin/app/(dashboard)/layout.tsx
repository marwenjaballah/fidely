...
import { AdminMobileHeader } from '@/components/mobile/admin-mobile-header'
import {
  SidebarProvider,
  SidebarInset,
  SidebarTrigger,
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  }, [isAuthenticated, hasHydrated, router, isMounted])

  const breadcrumbs = useMemo(() => {
    const items: Array<{ title: string; href: string; isLast: boolean }> = [
      {
        title: t('admin_badge_super_admin') || 'Super Admin',
        href: '/overview',
        isLast: pathname === '/' || pathname === '/overview',
      },
    ]

    if (pathname === '/' || pathname === '/overview') {
      items.push({
        title: t('admin_nav_overview') || 'Overview',
        href: '/overview',
        isLast: true,
      })
      return items
    ...
    if (pathname.startsWith('/stores')) {
      items.push({
        title: t('admin_nav_stores') || 'Stores',
        href: '/stores',
        isLast: true,
      })
    } else if (pathname.startsWith('/users')) {
      items.push({
        title: t('admin_nav_users') || 'Users',
        href: '/users',
        isLast: true,
      })
    } else if (pathname.startsWith('/transactions')) {
      items.push({
        title: t('admin_nav_transactions') || 'Transactions',
        href: '/transactions',
        isLast: true,
      })
    }
    ...
    return (
      <div className="min-h-svh flex items-center justify-center bg-background" dir={dir}>
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground font-medium">{t('admin_verifying') || 'Verifying credentials...'}</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-svh flex items-center justify-center bg-background" dir={dir}>
        <div className="text-center p-8 max-w-md">
          ...
          <h2 className="text-xl font-bold">{t('admin_restricted_title') || 'Access Restricted'}</h2>
          <p className="text-muted-foreground text-sm mt-2">
            {t('admin_restricted_desc') || 'Super Admin privileges are required to access this system.'}
          </p>
          <Button className="mt-6" onClick={() => router.push('/login')}>
            {t('admin_return_login') || 'Return to Login'}
          </Button>
        </div>
      </div>
    )
          ...
          <header className="sticky top-0 z-40 hidden md:flex h-12 md:h-14 lg:h-16 shrink-0 items-center gap-2 px-2 sm:px-4 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b">
            ...
            <div className="ms-auto flex items-center gap-1 sm:gap-2 shrink-0">
              ...
              >
                ...
                <span>{t('admin_global_admin_mode') || 'Platform Super Admin'}</span>
              </Badge>
